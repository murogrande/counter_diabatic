...
from sympy import I, S, symbols, diff, Function, simplify, Mul
...
def build_cd(n_qubits):
    ...
    d_xx = {(i,j): symbols(f"delta_xx_{i+1}{j+1}", real=True) for i,j in combinations(range(n_qubits),2)}
    d_yy = {(i,j): symbols(f"delta_yy_{i+1}{j+1}", real=True) for i,j in combinations(range(n_qubits),2)}
    d_zz = {(i,j): symbols(f"delta_zz_{i+1}{j+1}", real=True) for i,j in combinations(range(n_qubits),2)}
    d_xy = {(i,j): symbols(f"delta_xy_{i+1}{j+1}", real=True) for i,j in permutations(range(n_qubits),2)}
    d_xz = {(i,j): symbols(f"delta_xz_{i+1}{j+1}", real=True) for i,j in permutations(range(n_qubits),2)}
    d_yz = {(i,j): symbols(f"delta_yz_{i+1}{j+1}", real=True) for i,j in permutations(range(n_qubits),2)}
    ...
    H_lam = sum(Omega_l[i]*X[i] + mu_l[i]*Y[i] + nu_l[i]*Z[i] for i in range(n_qubits)) + sum(
        U[(i,j)] * two_body_op(s3, s3, i, j, n_qubits)
        for i, j in combinations(range(n_qubits), 2)
    )
    ...
    return CD, X, Y, Z, Omega, Mu, Nu, U, a, b, c, d_xx, d_yy, d_zz, d_xy, d_xz, d_yz, Omega_l, mu_l, nu_l, lam
...
def test_tp_compose_zero_after_pauli_cancel():
    ...
    comm = (I*Commutator(Z0, NOP0).expand(commutator=True)).expand(commutator=True).doit()
...
def test_tp_compose_scalar_times_add_of_tp():
    ...
    icomm = (I * Commutator(A_simple, H_nop).expand(commutator=True)).expand(commutator=True).doit()
...
def test_cd_assertions_n2():
    ...
    n = 2
    (CD, X, Y, Z, Omega, Mu, Nu, U,
     a, b, c, d_xx, d_yy, d_zz, d_xy, d_xz, d_yz,
     Omega_l, mu_l, nu_l, lam) = build_cd(n)

    norm = 2 ** (n + 1)

    expected = (-Omega[0]*d_yz[(1,0)] - Omega[1]*d_yz[(0,1)]
                + d_xy[(0,1)]*Nu[0] + d_xy[(1,0)]*Nu[1])
    assert simplify(tp_trace(tp_compose(Y[0]*Y[1], CD), n) / norm - expected) == 0

    expected2 = -b[0]*Nu[0] + c[0]*Mu[0] - d_yz[(0,1)]*U[(0,1)] + diff(Omega_l[0], lam) / 2
    assert (tp_trace(tp_compose(X[0], CD), n) / norm).equals(expected2)

    expected3 = a[1]*Nu[1] - c[1]*Omega[1] + d_xz[(1,0)]*U[(0,1)] + diff(mu_l[1], lam) / 2
